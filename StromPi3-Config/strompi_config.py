    ...
    if alarmconfig == 'Y':
        ...
        if alarmconfig == 'Y':
            ...
            while int(sp3_alarm_enable) < 0 or int(sp3_alarm_enable) > 1:
                sp3_alarm_enable = input('Failed! Value not in Range - Please try again\nWake up alarm status (0 - 1): ')

            if int(sp3_alarm_enable) == 1:
                print('\n--------------------------------------')
                print('Setting the wake up alarm mode (1 = Time-Alarm , 2 = Date-Alarm, 3 = Weekday-Alarm)\n')

                alarmMode = input('Wake up alarm mode (1 - 3): ')
                while int(alarmMode) < 1 or int(alarmMode) > 3:
                    alarmMode = input('Failed! Value not in Range - Please try again\nWake up alarm mode (1 - 3): ')
